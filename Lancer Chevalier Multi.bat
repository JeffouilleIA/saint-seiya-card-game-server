@echo off
chcp 65001 >nul
title Chevalier TCG — Multijoueur
cd /d "%~dp0"
...
echo Demarrage du serveur multijoueur Chevalier TCG...
start "Chevalier TCG — Serveur multijoueur" cmd /k "cd /d "%~dp0" && node server.js"

timeout /t 3 /nobreak >nul
start "" "http://127.0.0.1:3000"

echo.
echo Jeu ouvert : http://127.0.0.1:3000
echo Reseau   : voir la fenetre serveur pour l'adresse IP ^(meme Wi-Fi^)
echo Fermez la fenetre "Chevalier TCG — Serveur multijoueur" pour arreter.
